...
if (!array_key_exists('FLW_SSL_VERIFY', $cfg)) {
  $cfg['FLW_SSL_VERIFY'] = getenv('FLW_SSL_VERIFY') !== false
    ? filter_var(getenv('FLW_SSL_VERIFY'), FILTER_VALIDATE_BOOLEAN)
    : true;
}

if (!array_key_exists('FLW_CURL_CA_BUNDLE', $cfg)) {
  $cfg['FLW_CURL_CA_BUNDLE'] = getenv('FLW_CURL_CA_BUNDLE') ?: '';
}
