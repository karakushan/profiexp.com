...
class FirebaseService
{
  ...
  // Send push notification for vendor
  public static function sendToToken($title, $message, $token, array $data = [])
  {
    try {
      $app_firebase_json_file = DB::table('basic_settings')
        ->where('uniqid', 12345)
      ...
      $path = public_path('assets/file/') . $app_firebase_json_file;

      if (empty($app_firebase_json_file) || !file_exists($path)) {
        return false;
      }
