...
class ListingTranslationService
{
    ...
    public function translate(ListingContent $source, string $targetLangCode, string $targetLangName): array
    {
    ...
    public function buildTranslationPrompt(ListingContent $source, string $targetLangName): string
    {
        $fields = [];
        ...
        $fields['meta_keyword'] = $source->meta_keyword ?? '';
        $fields['meta_description'] = $source->meta_description ?? '';

        $json = json_encode($fields, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);

        return <<<PROMPT
Translate the following listing content to {$targetLangName}.
Preserve HTML tags in description field.
